...
HTTPSRedirect::HTTPSRedirect(const int p = 443) : _httpsPort(p) { Init(); }
...
// POST requests have 1 single blank like between the end of the header fields
...
void HTTPSRedirect::fetchHeader(void) {
  ...
  while (connected()) {
    ...
    if (pos < 0) {
      ...
      if (!pos)
        // get string & remove trailing '\r' character to facilitate string
        // comparisons
...
bool HTTPSRedirect::POST(const String &url, const char *host,
                         const String &payload, const bool &disp) {
  bool retval;
  bool oldval;
...
int HTTPSRedirect::getStatusCode(void) { return _myResponse.statusCode; }

String HTTPSRedirect::getReasonPhrase(void) { return _myResponse.reasonPhrase; }

String HTTPSRedirect::getResponseBody(void) { return _myResponse.body; }
